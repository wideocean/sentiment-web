...
public class SentimentServlet extends HttpServlet {
	...
	private SentimentService sentiHandler;
	private LanguageService langHandler;

	public void init(ServletConfig config) throws ServletException {
		ServletContext context = config.getServletContext();
		String absoluteDiskPath = context.getRealPath("");
		langHandler = new LanguageServiceImpl(absoluteDiskPath);
		sentiHandler = new SentimentServiceImpl(absoluteDiskPath);
		System.out.println("-----------------------------");
	...
	private void detectSentiment(HttpServletRequest request, HttpServletResponse response) throws IOException {

		String multiplereview = request.getParameter("reviewtext");
		String withKeywords = request.getParameter("keywords");

		JSONObject json = new JSONObject();
		if (multiplereview == null || multiplereview.equals("") || withKeywords == null || withKeywords.equals("")
				|| (!withKeywords.equals("true") && !withKeywords.equals("false"))) {
			...
			if (withKeywords.equals("false")) {
				...
				for (String e : reviews) {
					lineNumber++;
					String lang = langHandler.getLanguage(e);
